...
if IsDuplicityVersion() then
        ...
        for _, item in pairs(items) do
            if item.type == "weapon" then
                weapons[#weapons + 1] = {
                    name = item.name,
                    label = item.label,
                    ammo = item.info and item.info.ammo or 0,
                    components = item.info and item.info.components or {},
                    tintIndex = item.info and item.info.tintIndex or 0
    ...
    function Server.addWeaponComponent(source, weaponName, componentName)
        ...
        if item and item.type == "weapon" then
            local components = (item.info and item.info.components) or {}
    ...
    function Server.canSwapItem(source, firstItem, firstItemCount, secondItem, secondItemCount)
        local player = getPlayerFromId(source)
        if not player then error("Player not found") end
        -- Qbox doesn't have a direct swap function, check if we can carry both
    ...
    RegisterNetEvent("qbx_core:server:onPlayerUnload", function(src)
        for i = 1, #registeredCallbacks.onPlayerDropped do
            registeredCallbacks.onPlayerDropped[i](src)
        end
    end)

    RegisterNetEvent("qbx_core:server:onPlayerDeath", function(data)
        local src = source
    ...
    RegisterNetEvent("qbx_core:server:onJobUpdate", function(src, job)
